...
| 6 | Auth token stored in / retrieved from localStorage (SSR-safe) | ✓ VERIFIED | `src/lib/auth.ts`: `typeof window === 'undefined'` guard, key `cnb_auth_token`, exports getToken/setToken/clearToken/isAuthenticated |
...
| AUTH-02 | 01-01, 01-02, 01-03 | User can log in via /login and token persisted in localStorage | ✓ SATISFIED | `src/routes/login.tsx` → `useLoginMutation` → `setToken(data.token)` → `localStorage['cnb_auth_token']` |
...
**Test:** Set `localStorage['cnb_auth_token'] = 'test'` in browser console, then navigate to `/login`  
**Expected:** Immediately redirected to `/` without seeing the login page  
